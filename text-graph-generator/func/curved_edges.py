...
def curved_edges(G, pos, dist_ratio=0.2, bezier_precision=20, polarity='random'):
    ...
    edge_counter = {}
    edge_factor = []
    # Check occurences of edge
    for e in G.edges():
        if e[0]+'_'+e[1] in edge_counter:
            edge_counter[e[0]+'_'+e[1]] += 0.25
        else:
            edge_counter[e[0]+'_'+e[1]] = 1
        edge_factor.append(edge_counter[e[0]+'_'+e[1]])
    ...
    coord_diff = coords_node2[:,0]-coords_node1[:,0]
    coord_diff[coord_diff == 0] = 0.00001
    m1 = (coords_node2[:,1]-coords_node1[:,1])/coord_diff
    m1[m1 == 0] = 0.00001
    m2 = -1/m1
    ...
    coords_node1_displace = coords_node1 + (v1*t1*edge_factor).T 
    coords_node2_displace = coords_node2 - (v1*t1*edge_factor).T
